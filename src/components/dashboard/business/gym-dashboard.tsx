...
export default function GymDashboard({ stats, userName }: Props) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Link href="/pos" style={{ padding: '9px 16px', borderRadius: 10, background: '#F97316', color:'var(--text-1)', fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>
            Hap Arkën
          </Link>
          <Link href="/pos/products" style={{ padding: '9px 16px', borderRadius: 10, border: '1px solid var(--border)', background: 'var(--bg-card)', color: 'var(--text-2)', fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>
            Produktet
          </Link>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}>
        {[
          { label: 'Shitjet Sot',        value: fmt(stats.revenue),        color: '#F97316',  sub: 'abonime dhe shërbime' },
          { label: 'Kuponë Sot',        value: String(stats.transactions), color: '#10B981', sub: 'të shërbyer sot' },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              { label: 'Shto Shërbim / Abonim', href: '/pos/products', color: '#F97316'   },
              { label: 'Regjistro Shpenzim', href: '/expenses/new', color: '#3B82F6' },
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: 20 }}>
          ...
          {[
            { txt: 'TVSH 18% për shërbime fitness', color: '#F97316'   },
            { txt: 'Abonemenet 3/6/12 mujore — cash flow i qëndrueshëm', color: '#F59E0B' },
            { txt: 'Personal training — shërbim shtesë me marzh të lartë', color: '#EF4444' },
          ].map((t, i) => (
