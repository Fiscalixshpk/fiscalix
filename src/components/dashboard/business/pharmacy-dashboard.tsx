...
export default function PharmacyDashboard({ stats, userName }: Props) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Link href="/pos" style={{ padding: '9px 16px', borderRadius: 10, background:'var(--bg-card)', color:'var(--text-1)', fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>
            Hap Arkën
          </Link>
          <Link href="/suppliers" style={{ padding: '9px 16px', borderRadius: 10, border: '1px solid var(--border)', background: 'var(--bg-card)', color: 'var(--text-2)', fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>
            Furnizuesit
          </Link>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}>
        {[
          { label: 'Shitjet Sot',        value: fmt(stats.revenue),        color: '#0EA5E9',  sub: 'ilaçe dhe produkte' },
          { label: 'Receta Sot',        value: String(stats.transactions), color: '#10B981', sub: 'të shërbyer sot' },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              { label: 'Shto Ilaç / Produkt', href: '/pos/products', color: '#0EA5E9'   },
              { label: 'Blerje nga Furnizuesi', href: '/suppliers', color: '#3B82F6' },
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: 20 }}>
          ...
          {[
            { txt: 'TVSH 8% — ilaçe me recetë', color: '#0EA5E9'   },
            { txt: 'TVSH 18% — produkte OTC dhe kozmetikë', color: '#F59E0B' },
            { txt: 'Faturat e furnizuesit shkojnë te Libri Blerjeve', color: '#EF4444' },
          ].map((t, i) => (
