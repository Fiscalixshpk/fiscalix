...
const UNITS = ['muaj', 'projekt', 'orë', 'ditë', 'copë', 'postim', 'video', 'faqe']
const INVOICE_TYPES = ['Retainer Mujor', 'Faturë Projekti', 'Faturë Reklamash', 'Faturë Dizajni', 'Faturë Konsultimi', 'Faturë Shtesë']

interface Props { company: Company; userId: string; nextInvoiceNumber?: string }

export default function AgencyInvoiceForm({ company, userId, nextInvoiceNumber }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    client_vat: '',
    project_name: '',
    period: new Date().toLocaleString('sq-AL', { month:'long', year:'numeric' }),
    discount: '',
    notes: '',
  ...
  const totalWithVat = +(afterDiscount + vatAmount).toFixed(2)
        ...
        <div className="form-main">
          ...
          <div style={S}>
            <div className="form-grid-3">
              ...
              <div>
                <label style={L}>Nr. Projektit</label>
                <input value={form.project_number} onChange={e=>setForm(p=>({...p,project_number:e.target.value}))} placeholder="PRJ-2024/01" style={I}/>
              </div>
              <div>
                <label style={L}>Data Lëshimit</label>
                <input type="date" value={form.issue_date} onChange={e=>setForm(p=>({...p,issue_date:e.target.value}))} style={I}/>
              </div>
              <div>
                <label style={L}>Afati Pagesës</label>
                <input type="date" value={form.due_date} onChange={e=>setForm(p=>({...p,due_date:e.target.value}))} style={I}/>
              </div>
              <div>
                <label style={L}>Periudha</label>
                <input value={form.period} onChange={e=>setForm(p=>({...p,period:e.target.value}))} placeholder="Korrik 2026" style={I}/>
              </div>
            </div>
          </div>
          ...
          {/* Klienti */}
          <div style={S}>
            <h3 style={{ fontFamily:'Poppins,sans-serif', fontSize:14, fontWeight:700, color:'var(--text-1)', marginBottom:14 }}>Klienti / Brandi</h3>
            <div className="form-grid-2">
              <div className="form-span-2">
                <label style={L}>Emri / Kompania *</label>
                <input value={form.client_name} onChange={e=>setForm(p=>({...p,client_name:e.target.value}))} placeholder="Brand ABC SH.P.K" style={I}/>
              </div>
              <div>
                <label style={L}>Email</label>
                <input type="email" value={form.client_email} onChange={e=>setForm(p=>({...p,client_email:e.target.value}))} placeholder="finance@brand.com" style={I}/>
              </div>
              <div>
              ...
              <div className="form-span-2">
                ...
                <input value={form.project_name} onChange={e=>setForm(p=>({...p,project_name:e.target.value}))}
                  placeholder="Kampanja Vjeshtë 2026, Ribranding, Social Media Management..." style={I}/>
              </div>
            </div>
          </div>
          ...
          <div style={S}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14, flexWrap:'wrap', gap:8 }}>
              ...
              <ServicePicker companyId={company.id} onSelect={service => {
                setItems(prev => [...prev, { description: service.name, unit: 'muaj', quantity: 1, unit_price: service.price, total: service.price }])
              }}/>
            </div>
              ...
              <table style={{ width:'100%', borderCollapse:'collapse', minWidth:500 }}>
                <thead>
                  <tr style={{ borderBottom:'1px solid var(--border)', background:'var(--bg-muted)' }}>
                    {['Përshkrimi','Njësia','Sasia','Çmimi','Totali',''].map((h,i) => (
                    ...
                    <tr key={i} style={{ borderBottom:'1px solid var(--border)' }}>
                      <td style={{ padding:'6px 4px', minWidth:160 }}>
                        <input value={item.description} onChange={e=>updateItem(i,'description',e.target.value)}
                          placeholder="Social Media Management, Dizajn Logo..." style={{ ...I, fontSize:12 }}/>
                      </td>
                      ...
                      <td style={{ padding:'6px 4px', width:80 }}>
                        <input type="number" value={item.quantity||''} onChange={e=>updateItem(i,'quantity',parseFloat(e.target.value)||0)}
                          placeholder="1" style={{ ...I, fontSize:12, textAlign:'right' }}/>
                      </td>
                      <td style={{ padding:'6px 4px', width:110 }}>
                        <input type="number" value={item.unit_price||''} onChange={e=>updateItem(i,'unit_price',parseFloat(e.target.value)||0)}
                          placeholder="€" style={{ ...I, fontSize:12, textAlign:'right' }}/>
                      </td>
          ...
          <div style={S}>
            ...
            <textarea value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))}
              rows={2} placeholder="Kushtet e pagesës, deliverables, termat e kontratës..." style={{ ...I, resize:'none' }}/>
          </div>
        </div>
        ...
        <div className="form-sidebar">
          <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:14, padding:18, marginBottom:14 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:16 }}>
              <Palette size={15} style={{ color:'var(--text-1)' }}/>
            ...
            <div style={{ borderTop:'1px solid var(--border)', paddingTop:12, display:'flex', flexDirection:'column', gap:8 }}>
              ...
              <div style={{ borderTop:'2px solid #3B82F6', paddingTop:10, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                ...
                <span style={{ fontFamily:'Poppins,sans-serif', fontSize:22, fontWeight:900, color:'var(--text-1)' }}>€{totalWithVat.toFixed(2)}</span>
              </div>
            </div>
          </div>

          <button onClick={save} disabled={saving}
            style={{ width:'100%', padding:13, borderRadius:12, background:'linear-gradient(135deg,#1D4ED8,#3B82F6)', color:'white', fontSize:14, fontWeight:700, border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8, marginBottom:8 }}>
