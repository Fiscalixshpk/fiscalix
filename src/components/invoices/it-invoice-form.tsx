...
import { Plus, Trash2, Loader2, ArrowLeft, Code2 } from 'lucide-react'
...
const UNITS = ['orë', 'ditë', 'javë', 'muaj', 'projekt', 'licencë', 'server', 'domain', 'faqe', 'API']
const INVOICE_TYPES = ['Faturë Projekti', 'Mirëmbajtje Mujore', 'Licencë Software', 'Hosting & Server', 'Konsultim IT', 'Zhvillim Shtesë', 'Suport Teknik']
interface Props { company: Company; userId: string; nextInvoiceNumber?: string }

export default function ITInvoiceForm({ company, userId, nextInvoiceNumber }: Props) {
  const router = useRouter()
  const supabase = createClient()
  const today = new Date().toISOString().split('T')[0]
  const [form, setForm] = useState({
    invoice_number: nextInvoiceNumber || '', invoice_type: 'Faturë Projekti',
    project_number: '', issue_date: today,
    due_date: new Date(Date.now() + 15 * 86400000).toISOString().split('T')[0],
    client_name: '', client_email: '', client_address: '', client_vat: '',
    project_name: '', tech_stack: '', period: '', discount: '', notes: '',
    vat_rate: company.is_vat_registered ? 18 : 0,
  })
  const [items, setItems] = useState<LineItem[]>([{ description: '', unit: 'orë', quantity: 1, unit_price: 0, total: 0 }])
  ...
  function updateItem(i: number, field: keyof LineItem, value: string | number) {
    setItems(prev => prev.map((item, j) => {
      ...
      if (field === 'quantity' || field === 'unit_price') updated.total = +(Number(updated.quantity) * Number(updated.unit_price)).toFixed(2)
      return updated
    }))
  }
  ...
  async function save() {
    ...
    try {
      ...
      const notesText = [
        form.project_name ? `Projekti: ${form.project_name}` : '',
        form.tech_stack ? `Tech Stack: ${form.tech_stack}` : '',
        form.project_number ? `Nr. Projektit: ${form.project_number}` : '',
        form.period ? `Periudha: ${form.period}` : '',
        discount > 0 ? `Skonto: -€${discount.toFixed(2)}` : '',
        form.notes || '',
      ].filter(Boolean).join('\n')
      const { data: invoice, error } = await supabase.from('invoices').insert({
        company_id: company.id, created_by: user.id, invoice_number: form.invoice_number,
        issue_date: form.issue_date, due_date: form.due_date,
        client_name: form.client_name, client_email: form.client_email || null,
        client_address: form.client_address || null, client_vat: form.client_vat || null,
        status: 'pending', subtotal, discount_amount: discount,
        tax_rate: form.vat_rate, tax_amount: vatAmount, total_amount: total, total,
        payment_method: 'Bank Transfer', currency: 'EUR', notes: notesText || null,
      }).select().single()
        ...
        <div className="form-main">
          <div style={S}>
            <div className="form-grid-3">
              ...
              <div><label style={L}>Nr. Projektit</label><input value={form.project_number} onChange={e=>setForm(p=>({...p,project_number:e.target.value}))} placeholder="IT-2024/01" style={I}/></div>
              ...
              <div><label style={L}>Periudha</label><input value={form.period} onChange={e=>setForm(p=>({...p,period:e.target.value}))} placeholder="Korrik 2026" style={I}/></div>
            </div>
          </div>

          <div style={S}>
            <h3 style={{ fontFamily:'Poppins,sans-serif', fontSize:14, fontWeight:700, color:'var(--text-1)', marginBottom:14 }}>Klienti</h3>
            <div className="form-grid-2">
              <div className="form-span-2"><label style={L}>Emri / Kompania *</label><input value={form.client_name} onChange={e=>setForm(p=>({...p,client_name:e.target.value}))} placeholder="Kompania XYZ" style={I}/></div>
              <div><label style={L}>Email</label><input type="email" value={form.client_email} onChange={e=>setForm(p=>({...p,client_email:e.target.value}))} placeholder="cto@company.com" style={I}/></div>
              <div><label style={L}>NUI (opsionale)</label><input value={form.client_vat} onChange={e=>setForm(p=>({...p,client_vat:e.target.value}))} style={I}/></div>
              <div className="form-span-2"><label style={L}>Emri i Projektit</label><input value={form.project_name} onChange={e=>setForm(p=>({...p,project_name:e.target.value}))} placeholder="App Mobile, Web Platform, ERP..." style={I}/></div>
              <div className="form-span-2"><label style={L}>Tech Stack</label><input value={form.tech_stack} onChange={e=>setForm(p=>({...p,tech_stack:e.target.value}))} placeholder="React, Node.js, PostgreSQL, AWS..." style={I}/></div>
            </div>
          </div>

          <div style={S}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14, flexWrap:'wrap', gap:8 }}>
              <h3 style={{ fontFamily:'Poppins,sans-serif', fontSize:14, fontWeight:700, color:'var(--text-1)' }}>Shërbimet IT</h3>
              <ServicePicker companyId={company.id} onSelect={s => setItems(prev => [...prev, { description: s.name, unit: 'orë', quantity: 1, unit_price: s.price, total: s.price }])}/>
            </div>
            <div style={{ overflowX:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'collapse', minWidth:500 }}>
                <thead>
                  <tr style={{ borderBottom:'1px solid var(--border)', background:'var(--bg-muted)' }}>
                    {['Shërbimi / Task','Njësia','Sasia','Tarifa','Totali',''].map((h,i) => (
                ...
                <tbody>
                  {items.map((item, i) => (
                    <tr key={i} style={{ borderBottom:'1px solid var(--border)' }}>
                      <td style={{ padding:'6px 4px', minWidth:150 }}><input value={item.description} onChange={e=>updateItem(i,'description',e.target.value)} placeholder="Frontend Development..." style={{ ...I, fontSize:12 }}/></td>
                      ...
                      <td style={{ padding:'6px 4px', width:110 }}><input type="number" value={item.unit_price||''} onChange={e=>updateItem(i,'unit_price',parseFloat(e.target.value)||0)} placeholder="€" style={{ ...I, fontSize:12, textAlign:'right' }}/></td>
                      ...
                      <td style={{ padding:'6px 4px', width:36 }}>
                        {items.length > 1 && <button onClick={()=>setItems(prev=>prev.filter((_,j)=>j!==i))} style={{ padding:6, borderRadius:7, border:'1px solid rgba(239,68,68,0.2)', background:'rgba(239,68,68,0.07)', color:'var(--text-1)', cursor:'pointer', display:'flex' }}><Trash2 size={12}/></button>}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <button onClick={()=>setItems(prev=>[...prev,{description:'',unit:'orë',quantity:1,unit_price:0,total:0}])} style={{ display:'flex', alignItems:'center', gap:6, marginTop:10, padding:'7px 14px', borderRadius:9, border:'1px dashed rgba(16,185,129,0.35)', background:'transparent', color:'var(--text-1)', fontSize:12, fontWeight:600, cursor:'pointer' }}>
              <Plus size={13}/> Shto Task
            </button>
          </div>

          <div style={S}><label style={L}>Shënime teknike / SLA</label><textarea value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))} rows={2} placeholder="Deliverables, kushte SLA, termat e mirëmbajtjes..." style={{ ...I, resize:'none' }}/></div>
        </div>

        <div className="form-sidebar">
          <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:14, padding:18, marginBottom:14 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:16 }}>
              <Code2 size={15} style={{ color:'var(--text-1)' }}/>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:4, marginBottom:14 }}>
              {items.filter(i=>i.description&&i.total>0).map((item,i)=>(
                <div key={i} style={{ display:'flex', justifyContent:'space-between', fontSize:11, gap:8 }}>
                  <span style={{ color:'var(--text-3)', flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{item.description} ({item.quantity} {item.unit})</span>
          ...
          <button onClick={save} disabled={saving} style={{ width:'100%', padding:13, borderRadius:12, background:'linear-gradient(135deg,#059669,#10B981)', color:'white', fontSize:14, fontWeight:700, border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8, marginBottom:8 }}>
