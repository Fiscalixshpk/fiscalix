...
const UNITS = ['m²', 'm³', 'ml', 'copë', 'orë', 'ditë', 'ton', 'kg', 'litra', 'paketë']
const INVOICE_TYPES = ['Situatë', 'Faturë Finale', 'Avans', 'Situatë Materiale', 'Punë Shtesë']

interface Props { company: Company; userId: string; nextInvoiceNumber?: string }

export default function ConstructionInvoiceForm({ company, userId, nextInvoiceNumber }: Props) {
  ...
  const [items, setItems] = useState<LineItem[]>([
    { description: '', unit: 'm²', quantity: 0, unit_price: 0, total: 0 }
  ])
  ...
  async function save() {
    if (!form.client_name.trim()) { toast.error('Shto emrin e investitorit'); return }
    if (items.every(i => !i.description.trim())) { toast.error('Shto të paktën një zë'); return }
    setSaving(true)
    try {
      ...
      const notesText = [
        ...
        form.project_location ? `Lokacioni: ${form.project_location}` : '',
        form.contract_number ? `Nr. Kontratës: ${form.contract_number}` : '',
        advancePaid > 0 ? `Avans i paguar: €${advancePaid.toFixed(2)}` : '',
        form.notes || '',
      ].filter(Boolean).join('\n')
        ...
        <div className="form-main">
          ...
          <div style={S}>
            <div className="form-grid-3">
              <div>
                <label style={L}>Lloji i Dokumentit</label>
                <select value={form.invoice_type} onChange={e=>setForm(p=>({...p,invoice_type:e.target.value}))} style={I}>
                  {INVOICE_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div>
                <label style={L}>Numri</label>
                <input value={form.invoice_number} onChange={e=>setForm(p=>({...p,invoice_number:e.target.value}))} style={I}/>
              </div>
              <div>
                <label style={L}>Nr. Kontratës</label>
                <input value={form.contract_number} onChange={e=>setForm(p=>({...p,contract_number:e.target.value}))} placeholder="K-2024/01" style={I}/>
              </div>
              <div>
          ...
          {/* Investitori */}
          <div style={S}>
            <h3 style={{ fontFamily:'Poppins,sans-serif', fontSize:14, fontWeight:700, color:'var(--text-1)', marginBottom:14 }}>Investitori / Porosituesi</h3>
          ...
          {/* Projekti */}
          <div style={S}>
            <h3 style={{ fontFamily:'Poppins,sans-serif', fontSize:14, fontWeight:700, color:'var(--text-1)', marginBottom:14 }}>Projekti</h3>
            <div className="form-grid-2">
              <div>
                <label style={L}>Emri i Projektit</label>
                <input value={form.project_name} onChange={e=>setForm(p=>({...p,project_name:e.target.value}))} placeholder="Ndërtimi i Vilës" style={I}/>
              </div>
              <div>
                <label style={L}>Lokacioni</label>
                <input value={form.project_location} onChange={e=>setForm(p=>({...p,project_location:e.target.value}))} placeholder="Fushë Kosovë" style={I}/>
              </div>
            </div>
          </div>
          ...
          <div style={S}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14, flexWrap:'wrap', gap:8 }}>
              <h3 style={{ fontFamily:'Poppins,sans-serif', fontSize:14, fontWeight:700, color:'var(--text-1)' }}>Zërat e Punës</h3>
              <ServicePicker companyId={company.id} onSelect={service => {
                setItems(prev => [...prev, { description: service.name, unit: 'm²', quantity: 1, unit_price: service.price, total: service.price }])
              }}/>
            </div>
              ...
              <table style={{ width:'100%', borderCollapse:'collapse', minWidth:550 }}>
                <thead>
                  <tr style={{ borderBottom:'1px solid var(--border)', background:'var(--bg-muted)' }}>
                    {['Përshkrimi / Puna','Njësia','Sasia','Çmimi/Njësi','Totali',''].map((h,i) => (
                    ...
                    <tr key={i} style={{ borderBottom:'1px solid var(--border)' }}>
                      <td style={{ padding:'6px 4px', minWidth:160 }}>
                        <input value={item.description} onChange={e=>updateItem(i,'description',e.target.value)}
                          placeholder="p.sh. Betonizim themeli" style={{ ...I, fontSize:12 }}/>
                      </td>
            ...
            <button onClick={()=>setItems(prev=>[...prev,{description:'',unit:'m²',quantity:0,unit_price:0,total:0}])}
              ...
              <Plus size={13}/> Shto Zë Pune
            </button>
          </div>

          {/* Shënime */}
          <div style={S}>
            <label style={L}>Shënime shtesë</label>
            <textarea value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))}
              rows={2} placeholder="Kushte pagese, vërejtje teknike..." style={{ ...I, resize:'none' }}/>
          </div>
        </div>
        ...
        <div className="form-sidebar">
          <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:14, padding:18, marginBottom:14 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:16 }}>
              <Calculator size={15} style={{ color:'#F59E0B' }}/>
            ...
            <div style={{ borderTop:'1px solid var(--border)', paddingTop:12, display:'flex', flexDirection:'column', gap:8 }}>
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:12 }}>
                <span style={{ color:'var(--text-3)' }}>Nëntotali</span>
              ...
              {company.is_vat_registered && (
                <>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', fontSize:12 }}>
                    ...
                    <select value={form.vat_rate} onChange={e=>setForm(p=>({...p,vat_rate:parseInt(e.target.value)}))}
                      style={{ ...I, width:'auto', padding:'4px 8px', fontSize:11 }}>
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:12 }}>
                    ...
                    <span style={{ color:'#F59E0B', fontWeight:600 }}>€{vatAmount.toFixed(2)}</span>
                  </div>
                </>
              )}

              <div style={{ display:'flex', justifyContent:'space-between', fontSize:12 }}>
                <span style={{ color:'var(--text-3)' }}>Totali me TVSH</span>
                <span style={{ color:'var(--text-1)', fontWeight:700 }}>€{totalWithVat.toFixed(2)}</span>
              </div>

              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', fontSize:12 }}>
                <label style={{ color:'var(--text-3)' }}>(-) Avans i paguar</label>
              ...
              <div style={{ borderTop:'2px solid #F59E0B', paddingTop:10, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <span style={{ fontSize:14, fontWeight:800, color:'var(--text-1)' }}>PËR PAGESË</span>
                <span style={{ fontFamily:'Poppins,sans-serif', fontSize:22, fontWeight:900, color:'#F59E0B' }}>€{toPay.toFixed(2)}</span>
              </div>
            </div>
          </div>
          ...
          {company.is_vat_registered && (
            <div style={{ padding:'10px 14px', borderRadius:10, background:'rgba(245,158,11,0.06)', border:'1px solid rgba(245,158,11,0.2)', marginBottom:14, fontSize:11, color:'var(--text-3)', lineHeight:1.6 }}>
              Tatimi në Burim 9% aplikohet nëse porosituesi është subjekt tatimor
            </div>
          )}

          <button onClick={save} disabled={saving}
            style={{ width:'100%', padding:13, borderRadius:12, background:'linear-gradient(135deg,#B45309,#F59E0B)', color:'white', fontSize:14, fontWeight:700, border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8, marginBottom:8 }}>
