-- ═══════════════════════════════════════════════════════════
-- FISCALIX — FIX RLS KOMPLET
-- Ekzekuto këtë te Supabase SQL Editor
-- ═══════════════════════════════════════════════════════════
...
-- ─── FIX CASH_BANK_ENTRIES ───────────────────────────────
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'cash_bank_entries') THEN
    ALTER TABLE cash_bank_entries DISABLE ROW LEVEL SECURITY;
    ALTER TABLE cash_bank_entries ENABLE ROW LEVEL SECURITY;
    DROP POLICY IF EXISTS "cash_all" ON cash_bank_entries;
    CREATE POLICY "cash_all" ON cash_bank_entries FOR ALL USING (true);
  END IF;
END $$;

-- ─── FIX LIGHTWEIGHT TABLES ──────────────────────────────
...
-- ─── FIX ACCOUNTANT_SUBSCRIPTIONS ────────────────────────
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'accountant_subscriptions') THEN
    ...
    DROP POLICY IF EXISTS "acc_subs_all" ON accountant_subscriptions;
    CREATE POLICY "acc_subs_all" ON accountant_subscriptions FOR ALL USING (true);
  END IF;
END $$;

-- ─── FIX RECURRING_INVOICES ──────────────────────────────
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'recurring_invoices') THEN
    ALTER TABLE recurring_invoices DISABLE ROW LEVEL SECURITY;
    ALTER TABLE recurring_invoices ENABLE ROW LEVEL SECURITY;
    DROP POLICY IF EXISTS "recurring_all" ON recurring_invoices;
    CREATE POLICY "recurring_all" ON recurring_invoices FOR ALL USING (true);
  END IF;
END $$;

-- ─── PASTRO KLIENTËT E VJETËR ────────────────────────────
